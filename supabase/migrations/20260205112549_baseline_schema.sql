...
CREATE TABLE IF NOT EXISTS "public"."livestock_listings" (
    ...
    "invitation_id" "uuid",
    "profile_id" "uuid",
    "livestock_moved_location_to" "text",
    ...
    "additional_r25_per_head" boolean,
    "gln_num" "text",
    "previous_owner_declaration_url" "text",
...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();
  ...
  for insert
  to authenticated
with check (((bucket_id = 'livestock_affidavits'::text) AND (auth.uid() IS NOT NULL)));
  ...
  for insert
  to authenticated
with check (((bucket_id = 'documents'::text) AND (auth.uid() IS NOT NULL)));



  create policy "Authenticated users can view affidavits"
  on "storage"."objects"
  as permissive
  for select
  to authenticated
using (((bucket_id = 'livestock_affidavits'::text) AND (auth.uid() IS NOT NULL)));
...
using (((bucket_id = 'documents'::text) AND (auth.uid() IS NOT NULL)));
  ...
  create policy "Users can delete their own affidavits"
  on "storage"."objects"
  as permissive
  for delete
  to authenticated
...
using (((bucket_id = 'documents'::text) AND (auth.uid() IS NOT NULL)));
  ...
  create policy "Users can update their own affidavits"
  on "storage"."objects"
  as permissive
  for update
  to authenticated
